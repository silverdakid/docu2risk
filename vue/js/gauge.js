    ...
    BaseGauge = (function (superClass) {
        ...
        BaseGauge.prototype.configDisplayScale = function () {
            ...
            if (this.displayScale !== prevDisplayScale) {
                ...
                this.canvas.style.height = "175px";
    ...
    Gauge = (function (superClass) {
            ...
            for (j = 0, len = ref.length; j < len; j++) {
                ...
                if (value.label !== void 0) {
                    ...
                    if ((!this.options.limitMin || value >= this.minValue) && (!this.options.limitMax || value <= this.maxValue)) {
                        ...
                        this.ctx.rotate(rotationAngle);
                        // Ajout personnel pour mon cas :
                        if (value === staticLabels.score && value != 50) {
                            this.ctx.save()
                            this.ctx.beginPath();
                            this.ctx.fillStyle = staticLabels.scoreColor;
                            var s = 10 * this.displayScale
                            this.ctx.arc(0, -radius - this.lineWidth - 4.5, s, 0, 360)
                            this.ctx.fill();
                            this.ctx.restore();
                            this.ctx.fillText(value, 0, -radius - this.lineWidth + s / 4);
                        } else this.ctx.fillText((value === 50 ? "50+" : formatNumber(value, staticLabels.fractionDigits)), 0, -radius - this.lineWidth / 2);
                        this.ctx.rotate(-rotationAngle);
                    }
                }
            }
        ...
        Gauge.prototype.render = function () {
            ...
            if (this.options.staticZones) {
                ...
                // perso : AJOUT DES TEXTES :
                const levels = [{ name: "LOW", pos: 9 }, { name: "MEDIUM", pos: -5 }, { name: "HIGH", pos: 35 }]
                this.ctx.save();
                this.ctx.font = `bold ${12 * this.displayScale}px sans-serif`;
                this.ctx.textBaseline = "top";
                this.ctx.textAlign = "center";
                for (const level of levels) {
                    rotationAngle = (level.name === "MEDIUM") ? 0 : this.getAngle(level.pos) - 3 * Math.PI / 2;
                    if (level.name === "HIGH") {
                        var t = 45 * Math.PI / 180;
                        this.ctx.rotate(t);
                        this.ctx.fillText("HIGH", 15, (-radius - 5))
                        this.ctx.rotate(-t);
                    } else {
                        this.ctx.rotate(rotationAngle)
                        this.ctx.fillText(level.name, level.pos, (-radius - 3))
                        this.ctx.rotate(-rotationAngle);
                    }
                }
                this.ctx.restore()
